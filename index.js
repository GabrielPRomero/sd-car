...
const traffic = [
  ...
  new Car(road.getLaneCenter(0), -900, 30, 50, "DUMMY", 2.7),
  new Car(road.getLaneCenter(1), -800, 30, 50, "DUMMY", 2.9),
  new Car(road.getLaneCenter(2), -800, 30, 50, "DUMMY", 2.8),
];

animate();

function save() {
  localStorage.setItem("bestBrain", JSON.stringify(mainCar.brain));
}
...
function animate(time) {
  ...
  mainCar = cars.find((car) => car.y == Math.min(...cars.map((car) => car.y)));
  carCanvas.height = window.innerHeight;
  networkCanvas.height = window.innerHeight;

  carCtx.save();
  carCtx.translate(0, -mainCar.y + carCanvas.height * 0.7);
